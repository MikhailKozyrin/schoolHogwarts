...
public class SchoolHogwartsApplicationMvcTests {
    @Autowired
    ...
    public void testEditFaculty() throws Exception {
        ...
        when(facultyRepository.findById(id)).thenReturn(Optional.of(faculty));
        when(facultyRepository.save(ArgumentMatchers.any(Faculty.class))).thenReturn(updatedFaculty);
        ...
        mockMvc.perform(MockMvcRequestBuilders
                ...
                .andExpect(status().isOk())
                .andExpect((ResultMatcher) jsonPath("$.id").value(id))
                .andExpect((ResultMatcher) jsonPath("$.name").value(newName))
                .andExpect((ResultMatcher) jsonPath("$.color").value(newColor));
    }
